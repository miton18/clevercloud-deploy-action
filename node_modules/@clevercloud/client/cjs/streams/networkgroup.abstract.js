...
exports.AbstractNetworkgroupStream = void 0;
...
class AbstractNetworkgroupStream extends _streamAbstract.AbstractStream {
  /**
  ...
  prepareWireguardConfigurationSse() {
    return ngApi.getNetworkGroupWireGuardConfigurationStream({
      ...
      ng_id: this.ngId,
      peer_id: this.peerId
    }).then((0, _prefixUrl.prefixUrl)(this.apiHost)).then(this.addOauthHeader(this.tokens)).then(requestParams => {
  ...
  async _openSource() {
    const {
      url
    } = await this.prepareWireguardConfigurationSse();
    ...
    }, OPEN_TIMEOUT); // Wire SSE "message" to _onPing() call or "conf" event
    ...
    this._sse.addEventListener('error', error => {
      clearTimeout(openTimeoutId);
      this.isAuthErrorMessage(error) ? this._onError(new _streamAbstract.AuthenticationError('Authentication for the networkgroup configuration stream (SSE) failed')) : this._onError(error);
    });
  }
...
exports.AbstractNetworkgroupStream = AbstractNetworkgroupStream;