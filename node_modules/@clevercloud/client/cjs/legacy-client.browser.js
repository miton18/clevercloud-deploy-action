...
var _oauthBrowser = require("./oauth.browser.js");
...
var _requestFetch = require("./request.fetch.js");
...
function initCleverClient(config) {
  const legacyClient = (0, _legacyClient.initLegacyClient)((clientFn, pathParamNames = []) => {
    return () => (0, _sendLegacyRequest.sendLegacyRequest)((req, body) => {
      ...
      const restCallPromise = clientFn(params, body).then(requestParams => {
        return { ...requestParams,
          headers: { ...requestParams.headers,
            ...req.headers
          }
        };
      }).then((0, _prefixUrl.prefixUrl)(config.API_HOST)).then((0, _oauthBrowser.addOauthHeader)(config)).then(_requestFetch.request);
